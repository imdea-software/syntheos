...
Surprise! It is z3. This module is the seam between the rest of the shield and
...
def isSat(formula: BoolRef) -> bool:
    ...
    if result == unsat:
        return False
    raise ShieldError("Unknown satisfiability")
...
def makevar(var: str, ty: str) -> ArithRef:
    match ty:
        ...
        case "Real":
            cons = Real
        case _:
            raise ShieldError("Unhandled type: " + ty)
    return cons(var)
...
def z32ltltw(f: ExprRef, funs: dict[str, Any]) -> Any:
    ...
    def convert(f: ExprRef) -> Any:
        ...
        if is_quantifier:
            ...
            return ltlTh(f)
        raise ShieldError("Unhandled case:" + str(f))  # unreachable, see above
